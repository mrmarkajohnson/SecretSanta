...
public class LoginQuery : BaseQuery<SignInResult>
{
    ...
    public async override Task<SignInResult> Handle()
    {
        HashedUserId hashedId = await Send(new GetHashedIdQuery(_item.EmailOrUserName, false));
        SignInResult result = await _signInManager.PasswordSignInAsync(hashedId.UserNameHash, _item.Password, _item.RememberMe, lockoutOnFailure: false);
        return result;
    }
}
